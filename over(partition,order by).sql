...
create database windows10db;
 use windows10db;
 CREATE TABLE employee_performance ( employee_id INT, employee_name VARCHAR(50),
 department VARCHAR(30), location VARCHAR(30),
 performance_year INT, performance_month INT,
 salary DECIMAL(10,2), sales_amount DECIMAL(12,2),
 rating DECIMAL(3,1) );
 INSERT INTO employee_performance (employee_id, employee_name,
 department, location, performance_year, performance_month,
 salary, sales_amount, rating) VALUES (101, 'Amit', 'Sales', 'Jaipur', 2025, 1, 45000, 120000, 4.2),
 ...
 select *, sum(sales_amount) over(order by sales_amount) from employee_performance;
  ...
  select *, sum(sales_amount) over(order by employee_name) from employee_performance;
  
   -- rank(), dense rank(),n row_number()
